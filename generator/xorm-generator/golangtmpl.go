...
var (
	genJson                                      bool     = false
	ignoreColumnsJSON, created, updated, deleted []string = []string{}, []string{"created"}, []string{"updated"}, []string{"isDeleted"}
)
var (
	supportComment bool
...
func tag(table *core.Table, col *core.Column) string {
	...
	//}
	if include(updated, col.Name) {
		res = append(res, "updated")
	}
	
	if  include(deleted, col.Name) {
